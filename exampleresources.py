...
class JSONResource(Resource):
    def __init__(self, name="JSON"):
        ...
        self.value = 0
        my_json_string = json.dumps({'Temperature': self.value})
        self.payload = (defines.Content_types["application/json"], my_json_string)
    ...
    def render_PUT(self, request):
        #self.payload = request.payload
        if isValid == validateJSON(request.payload):
            self.payload = request.payload
        else:
            self.payload = "Not a JSON format compliant"
        return self
